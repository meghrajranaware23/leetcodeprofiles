/* ══════════════════════════════════════════════════════════
   GRAPHS READER — JavaScript Module
   Graphs Ascension · Solo Leveling Theme
   ══════════════════════════════════════════════════════════ */
...
const diagramEnhancer = createDiagramEnhancer('graphs');
diagramEnhancer.bindContent(contentEl);

const PACK_ID = 'graphs';
...
/* ─── Init ─── */
function init() {
  document.body.dataset.pack = 'graphs';
  initReaderNav('graphs');
  loadProgress();
  migrateLegacyProgress();
  ...
  if (completed >= total - 1) {
    ...
    const awakening = rank === 's'
      ? {
          ...
          detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. 33 patterns mastered. 78+ problems solved. The territory ascension is complete.`,
        }
      : rank === 'a'
        ? {
            icon: '🟠→🔴',
...
function showMilestone(lesson) {
  ...
  if (lesson.id === 'rank-s-complete') {
    ...
    milestoneMsg.textContent = 'The territory ascension is complete. 33 patterns mastered. You are Graph Legend.';
  }
