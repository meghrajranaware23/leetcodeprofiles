/* ══════════════════════════════════════════════════════════
   RECURSION PROGRESS — Pack-scoped progress API
   Recursion & Backtracking Ascension · uses progress-store.js
   ══════════════════════════════════════════════════════════ */
...
export const PACK_ID = PACK_IDS.RECURSION;
...
const ACHIEVEMENT_DEFS = {
  'briefing-complete': { title: 'Stack Awakening', icon: '🔄' },
  'first-concept': { title: 'First Frame Pushed', icon: '📚' },
  'first-quest': { title: 'First Recursive Quest', icon: '⚔' },
  'first-checkpoint': { title: 'Path Traced', icon: '✅' },
  'first-test': { title: 'First Trial Passed', icon: '🎯' },
  'e-rank-complete': { title: 'E-Rank: Novice', icon: '🏆' },
  'stack-tracer': { title: 'Stack Tracer', icon: '📊' },
  'trust-master': { title: 'Trust Master', icon: '🤝' },
  'first-d-rank-concept': { title: 'First D-Rank Pattern', icon: '🔵' },
  'divide-conqueror': { title: 'Divide & Conqueror', icon: '✂️' },
  'd-rank-complete': { title: 'D-Rank: Apprentice', icon: '🏗️' },
  'first-c-rank-concept': { title: 'First C-Rank Pattern', icon: '🟢' },
  'backtracking-unlocked': { title: 'Backtracking Unlocked', icon: '🌿' },
  'permutation-master': { title: 'Permutation Master', icon: '🔀' },
...
export function recordLessonAchievements(lesson) {
  ...
  if (lesson.id === '2-4') recordAchievement('stack-tracer');
  if (lesson.id === '5-4') recordAchievement('trust-master');
  if (lesson.id === '8-4') recordAchievement('divide-conqueror');
  if (lesson.id === '11-4') recordAchievement('backtracking-unlocked');
  if (lesson.id === '12-4') recordAchievement('permutation-master');
  if (lesson.id === '16-4') recordAchievement('grid-explorer');
  if (lesson.id === '17-4') recordAchievement('pruning-expert');
  if (lesson.id === '18-4') recordAchievement('constraint-solver');
  if (lesson.id === '23-4') recordAchievement('memo-master');
  if (lesson.id === '28-3') recordAchievement('recursive-synthesizer');
