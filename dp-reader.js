/* ══════════════════════════════════════════════════════════
   DP READER — JavaScript Module
   Dynamic Programming Ascension · Solo Leveling Theme
   ══════════════════════════════════════════════════════════ */
...
const PACK_ID = 'dynamic-programming';
  ...
  if (completed >= total - 1) {
    ...
    const awakening = rank === 's'
      ? {
          ...
          detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. 32 patterns mastered. The dynamic ascension is complete.`,
        }
      : rank === 'a'
        ? {
            icon: '🟠→🔴',
            title: 'A-Rank Master Confirmed',
            detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. A-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — pattern synthesis instinctive.`,
          }
        : rank === 'b'
        ? {
            icon: '🟡→🟠',
            title: 'B-Rank Strategist Confirmed',
            detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. B-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — knapsack and state machines mastered.`,
          }
        : rank === 'c'
        ? {
            icon: '🟢→🟡',
            title: 'C-Rank Pathfinder Confirmed',
            detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. C-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — subsequences and grids conquered.`,
          }
        : rank === 'd'
          ? {
              icon: '🔵→🟢',
              title: 'D-Rank Apprentice Confirmed',
              detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. D-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — decision DP mastered.`,
            }
          : {
              icon: '⬛→🔵',
...
function showMilestone(lesson) {
  ...
  if (lesson.id === 'rank-e-complete') {
    ...
    milestoneMsg.textContent = 'You see overlapping subproblems. D-Rank is unlocked — learn to decide.';
  }
  if (lesson.id === 'rank-d-complete') {
    milestoneTitle.textContent = 'D-Rank Apprentice Confirmed!';
    milestoneMsg.textContent = 'Decision DP mastered. C-Rank is unlocked — learn to navigate 2D.';
  }
  if (lesson.id === 'rank-c-complete') {
    milestoneTitle.textContent = 'C-Rank Pathfinder Confirmed!';
    milestoneMsg.textContent = 'Subsequences and grids conquered. B-Rank is unlocked — learn to transform.';
  }
  if (lesson.id === 'rank-b-complete') {
    milestoneTitle.textContent = 'B-Rank Strategist Confirmed!';
    milestoneMsg.textContent = 'Knapsack and state machines mastered. A-Rank is unlocked — learn to synthesize.';
  }
  if (lesson.id === 'rank-a-complete') {
    milestoneTitle.textContent = 'A-Rank Master Confirmed!';
    milestoneMsg.textContent = 'Pattern synthesis instinctive. S-Rank is unlocked — the dynamic ascension awaits.';
  }
  if (lesson.id === 'rank-s-complete') {
    milestoneTitle.textContent = 'S-Rank Legend Confirmed!';
    milestoneMsg.textContent = 'The dynamic ascension is complete. 32 patterns mastered. You are Dynamic Legend.';
  }
