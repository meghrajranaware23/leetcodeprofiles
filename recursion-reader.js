/* ══════════════════════════════════════════════════════════
   RECURSION READER — JavaScript Module
   Recursion & Backtracking Ascension · Solo Leveling Theme
   ══════════════════════════════════════════════════════════ */
...
const RANK_META = {
  ...
  e:      { label: '⬛ E-RANK — NOVICE',       color: '#666666', order: 1 },
  d:      { label: '🔵 D-RANK — APPRENTICE',  color: '#4A9EFF', order: 2 },
  c:      { label: '🟢 C-RANK — PATHFINDER',  color: '#4AFF9E', order: 3 },
  b:      { label: '🟡 B-RANK — STRATEGIST',  color: '#FFD700', order: 4 },
  a:      { label: '🟠 A-RANK — MASTER',      color: '#FF6B35', order: 5 },
...
const diagramEnhancer = createDiagramEnhancer('recursion');
diagramEnhancer.bindContent(contentEl);

const PACK_ID = 'recursion';
...
/* ─── Init ─── */
function init() {
  document.body.dataset.pack = 'recursion';
  initReaderNav('recursion');
  loadProgress();
  migrateLegacyProgress();
  ...
  if (completed >= total - 1) {
    ...
    const awakening = rank === 's'
      ? {
          ...
          detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. 27 patterns mastered. 78 problems solved. The recursive ascension is complete.`,
        }
      : rank === 'a'
        ? {
            icon: '🟠→🔴',
            title: 'A-Rank Master Confirmed',
        ...
        ? {
            icon: '🟡→🟠',
            title: 'B-Rank Strategist Confirmed',
            detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. B-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — pruning and constraints mastered.`,
          }
        : rank === 'c'
        ? {
            icon: '🟢→🟡',
            title: 'C-Rank Pathfinder Confirmed',
            detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. C-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — backtracking template owned.`,
          }
        : rank === 'd'
          ? {
              icon: '🔵→🟢',
              title: 'D-Rank Apprentice Confirmed',
              detail: `You've earned <strong>${xp.toLocaleString()} XP</strong> total. D-Rank: ${stats.quests} quests and ${stats.tests} tests cleared — divide and conquer unlocked.`,
            }
          : {
              icon: '⬛→🔵',
              title: 'E-Rank Novice Confirmed',
...
function showMilestone(lesson) {
  ...
  if (lesson.id === 'rank-e-complete') {
    milestoneTitle.textContent = 'E-Rank Novice Confirmed!';
    milestoneMsg.textContent = 'You trust the call stack. D-Rank is unlocked — learn to divide.';
  }
  if (lesson.id === 'rank-d-complete') {
    milestoneTitle.textContent = 'D-Rank Apprentice Confirmed!';
    milestoneMsg.textContent = 'Divide and conquer mastered. C-Rank is unlocked — learn to explore.';
  }
  if (lesson.id === 'rank-c-complete') {
    milestoneTitle.textContent = 'C-Rank Pathfinder Confirmed!';
    milestoneMsg.textContent = 'Backtracking template owned. B-Rank is unlocked — learn to prune.';
  }
  if (lesson.id === 'rank-b-complete') {
    milestoneTitle.textContent = 'B-Rank Strategist Confirmed!';
    milestoneMsg.textContent = 'Pruning and constraints mastered. A-Rank is unlocked — learn to optimize.';
  }
  if (lesson.id === 'rank-a-complete') {
    milestoneTitle.textContent = 'A-Rank Master Confirmed!';
    milestoneMsg.textContent = 'Memoization and synthesis instinctive. S-Rank is unlocked — the final ascension awaits.';
  }
  if (lesson.id === 'rank-s-complete') {
    milestoneTitle.textContent = 'S-Rank Legend Confirmed!';
    milestoneMsg.textContent = 'The recursive ascension is complete. 27 patterns mastered. You are Recursive Legend.';
  }
