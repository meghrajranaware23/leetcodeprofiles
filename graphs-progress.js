/* ══════════════════════════════════════════════════════════
   GRAPHS PROGRESS — Pack-scoped progress API
   Graphs Ascension · uses progress-store.js
   ══════════════════════════════════════════════════════════ */

import { COURSE_LESSONS } from './graphs-content.js';
import {
  PACK_IDS,
  PACK_REGISTRY,
  ROOT_SCHEMA_VERSION,
...
export const PACK_ID = PACK_IDS.GRAPHS;
...
const ACHIEVEMENT_DEFS = {
  'briefing-complete': { title: 'Territory Scouted', icon: '🕸️' },
  'first-concept': { title: 'First Node Discovered', icon: '📍' },
  'first-quest': { title: 'First Edge Traversed', icon: '⚔' },
  'first-checkpoint': { title: 'Path Mapped', icon: '✅' },
  'first-test': { title: 'First Expedition Passed', icon: '🎯' },
  'e-rank-complete': { title: 'E-Rank: Scout', icon: '🏆' },
  'traversal-master': { title: 'Traversal Navigator', icon: '🧭' },
  'grid-explorer': { title: 'Grid Explorer', icon: '🗺️' },
  'first-d-rank-concept': { title: 'First D-Rank Pattern', icon: '🔵' },
  'boundary-mapper': { title: 'Boundary Mapper', icon: '🌊' },
  'state-navigator': { title: 'State Navigator', icon: '🔐' },
  'd-rank-complete': { title: 'D-Rank: Pathfinder', icon: '🏗️' },
  'first-c-rank-concept': { title: 'First C-Rank Pattern', icon: '🟢' },
  'cycle-detector': { title: 'Cycle Detector', icon: '🔄' },
  'topo-sort-master': { title: 'Order Discoverer', icon: '📋' },
  'bipartite-spotter': { title: 'Two-Color Thinker', icon: '🎨' },
  'c-rank-complete': { title: 'C-Rank: Cartographer', icon: '🛡️' },
  'first-b-rank-concept': { title: 'First B-Rank Pattern', icon: '🟡' },
  'territory-unifier': { title: 'Territory Unifier', icon: '🔗' },
  'shortest-path-master': { title: 'Shortest Path Master', icon: '⚡' },
  'b-rank-complete': { title: 'B-Rank: Navigator', icon: '⭐' },
  'first-a-rank-concept': { title: 'First A-Rank Pattern', icon: '🟠' },
  'word-ladder-solved': { title: 'Word Transformer', icon: '🪜' },
  'a-rank-complete': { title: 'A-Rank: Expedition Leader', icon: '🧭' },
  'first-s-rank-concept': { title: 'First S-Rank Strategy', icon: '🔴' },
  'graph-synthesizer': { title: 'Graph Synthesizer', icon: '🧬' },
  'ascension-master': { title: 'Ascension Master', icon: '⛰️' },
  'final-test-cleared': { title: 'Final Test Cleared', icon: '💀' },
  'legend': { title: 'Graph Legend', icon: '👑' },
};
...
export function recordLessonAchievements(lesson) {
  ...
  if (lesson.id === '4-4') recordAchievement('grid-explorer');
  if (lesson.id === '7-4') recordAchievement('boundary-mapper');
  if (lesson.id === '10-4') recordAchievement('state-navigator');
  if (lesson.id === '11-4') recordAchievement('cycle-detector');
  if (lesson.id === '12-4') recordAchievement('topo-sort-master');
  if (lesson.id === '13-4') recordAchievement('bipartite-spotter');
  if (lesson.id === '18-4') recordAchievement('territory-unifier');
  if (lesson.id === '20-4') recordAchievement('shortest-path-master');
  if (lesson.id === '23-2') recordAchievement('word-ladder-solved');
  if (lesson.id === '29-3') recordAchievement('graph-synthesizer');
