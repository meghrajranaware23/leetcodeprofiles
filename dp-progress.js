/* ══════════════════════════════════════════════════════════
   DP PROGRESS — Pack-scoped progress API
   Dynamic Programming Ascension · uses progress-store.js
   ══════════════════════════════════════════════════════════ */
...
export const PACK_ID = PACK_IDS.DP;
...
const ACHIEVEMENT_DEFS = {
  'briefing-complete': { title: 'DP Awakening', icon: '📊' },
  'first-concept': { title: 'First State Defined', icon: '📚' },
  'first-quest': { title: 'First Optimal Quest', icon: '⚔' },
  'first-checkpoint': { title: 'Transition Traced', icon: '✅' },
  'first-test': { title: 'First Trial Passed', icon: '🎯' },
  'e-rank-complete': { title: 'E-Rank: Novice', icon: '🏆' },
  'pipeline-master': { title: 'Pipeline Master', icon: '🔄' },
  'table-builder': { title: 'Table Builder', icon: '📋' },
  'first-d-rank-concept': { title: 'First D-Rank Pattern', icon: '🔵' },
  'decision-maker': { title: 'Decision Maker', icon: '🤔' },
  'd-rank-complete': { title: 'D-Rank: Apprentice', icon: '🏗️' },
  'first-c-rank-concept': { title: 'First C-Rank Pattern', icon: '🟢' },
  'grid-navigator': { title: 'Grid Navigator', icon: '🗺️' },
  'subsequence-hunter': { title: 'Subsequence Hunter', icon: '🎯' },
  'palindrome-detector': { title: 'Palindrome Detector', icon: '🔍' },
  'c-rank-complete': { title: 'C-Rank: Pathfinder', icon: '🛡️' },
  'first-b-rank-concept': { title: 'First B-Rank Pattern', icon: '🟡' },
  'knapsack-strategist': { title: 'Knapsack Strategist', icon: '🎒' },
  'coin-master': { title: 'Coin Master', icon: '🪙' },
  'state-machine-operator': { title: 'State Machine Operator', icon: '⚙️' },
  'edit-distance-pro': { title: 'Edit Distance Pro', icon: '✏️' },
  'b-rank-complete': { title: 'B-Rank: Strategist', icon: '⭐' },
  'first-a-rank-concept': { title: 'First A-Rank Pattern', icon: '🟠' },
  'pattern-synthesizer': { title: 'Pattern Synthesizer', icon: '🧬' },
  'a-rank-complete': { title: 'A-Rank: Master', icon: '🌟' },
  'first-s-rank-concept': { title: 'First S-Rank Strategy', icon: '🔴' },
  'dp-synthesizer': { title: 'DP Synthesizer', icon: '💎' },
  'ascension-master': { title: 'Ascension Master', icon: '⛰️' },
  'final-test-cleared': { title: 'Final Test Cleared', icon: '💀' },
  'legend': { title: 'Dynamic Legend', icon: '👑' },
};
...
export function recordLessonAchievements(lesson) {
  ...
  if (lesson.id === '3-4') { recordAchievement('pipeline-master'); recordAchievement('table-builder'); }
  if (lesson.id === '6-4') recordAchievement('decision-maker');
  if (lesson.id === '11-4') recordAchievement('grid-navigator');
  if (lesson.id === '12-4') recordAchievement('subsequence-hunter');
  if (lesson.id === '14-4') recordAchievement('palindrome-detector');
  if (lesson.id === '17-4') recordAchievement('knapsack-strategist');
  if (lesson.id === '18-4') recordAchievement('coin-master');
  if (lesson.id === '20-4') recordAchievement('state-machine-operator');
  if (lesson.id === '21-4') recordAchievement('edit-distance-pro');
  if (lesson.id === '26-4') recordAchievement('pattern-synthesizer');
  if (lesson.id === '29-4') recordAchievement('dp-synthesizer');
