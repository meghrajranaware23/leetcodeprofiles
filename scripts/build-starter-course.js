...
function buildContent() {
  ...
  for (const day of DAYS) {
    track(writeLesson(day.concept.file, genGuide(day)));
    track(writeLesson(`${String(day.day).padStart(2, '0')}-4-checkpoint.md`, genCheckpoint(day)));
    day.quests.forEach((q, i) => {
      track(writeLesson(q.file, genQuest(day, q, i + 1, day.day <= 8), { type: 'quest', lc: q.lc }));
    });
    if (day.setupMission) {
      track(writeLesson(day.setupMission.file, genSetupMission(day)));
    }
  }

  for (const [phase, block] of Object.entries(PHASE_TESTS)) {
    block.tests.forEach((t, i) => {
      track(writeLesson(t.file, genTest(phase, t, i, block.tests.length), { type: 'test', lc: t.lc }));
    });
    track(writeLesson(block.complete.file, genPhaseComplete(phase)));
  }

  console.log(`Build: ${stats.wrote} wrote, ${stats.skipped} skipped (hand-authored), ${stats.patched} solution-patched`);
  console.log(`${fs.readdirSync(MICRO_DIR).length} markdown files in course/starter/micro/`);
}
